...
// Each thread computes one hidden element: hidden[b,h]
...
// Each thread computes one output: out[b,c]
...
int main(int argc, char** argv) {
    ...
    // Device weights (copied once; excluded from timing)
    ...
    // Chunked batch to avoid huge device buffers
    int chunkB = 256;
    ...
    for (int base = 0; base < B; base += chunkB) {
        ...
        // fill X chunk on host deterministically
        for (int b = 0; b < Bc; b++) {
            int gb = base + b;
            for (int d = 0; d < D; d++) {
                hX[(size_t)b * D + d] = gen_input(gb, d);
            }
        }
        ...
        for (int i = 0; i < Bc * C; i++) checksum += (double)hOut[(size_t)i];
    }

    ck(cudaDeviceSynchronize(), "sync after loop");
